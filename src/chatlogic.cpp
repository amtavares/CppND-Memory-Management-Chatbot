...
ChatLogic::ChatLogic()
{
    ...
    // _chatBot = new ChatBot("../images/chatbot.png"); // Removed for task 5
...
ChatLogic::~ChatLogic()
{
    //// STUDENT CODE
    ////
    /*   Delete is not necessary for unique_ptr, task 3
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            //Example from udacity knowledge base:
                            //auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> &node) { return node->GetID() == id; });
                            ...
                            // create new edge
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id); //Making unique pointer [task 4]
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get());       //adapting for unique pointer
                            (*parentNode)->AddEdgeToChildNode(std::move(edge));  //move semantics, task 4
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                rootNode = (*it).get(); // assign current node to root // changed for unique pointer
            }
            else
            {
    ...
    ChatBot chatBot("../images/chatbot.png");       //task 5, creating ChatBot instance here
    SetChatbotHandle(&chatBot);
    chatBot.SetChatLogicHandle(this);
    chatBot.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(chatBot));   //using move semantics
